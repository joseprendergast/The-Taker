...
public partial class PowerQuestEditor : EditorWindow 
{
	...
	Vector2 m_guiMousePosCached = Vector2.zero;
	...
	private static void CreateInventoryItem()
	{
		CreateQuestObjectWindow.Show( eQuestObjectType.Inventory, PowerQuest.STR_INVENTORY, 
			"'RubberChicken' or 'TreasureMap'", CreateInventory);
	}
	...
	private static void CreateGuiItem()
	{
		CreateQuestObjectWindow.Show( eQuestObjectType.Gui, PowerQuest.STR_GUI, 
			"'Toolbar' or 'InventoryBox'", new CreateQuestObjectWindow.DelegateCreateFunction(CreateGui));
	}
	...
	public bool SanitizeQuestObjectName(eQuestObjectType questType, ref string name, out string error )
	{
		...
		/*
		Room inRoom = null;
		if ( questType == eQuestObjectType.Hotspot || questType == eQuestObjectType.Prop || questType == eQuestObjectType.Region )
			inRoom = true;*/
	...
	public List<IQuestScriptable> GetQuestScriptables( eQuestObjectType type )
	{		
		//if ( inRoom == null )
		...
		case eQuestObjectType.Prop:	     return (new List<PropComponent>(m_selectedRoom?.GetComponentsInChildren<PropComponent>(true))).ConvertAll<IQuestScriptable>(item=>item.GetData());
		case eQuestObjectType.Hotspot:   return (new List<HotspotComponent>(m_selectedRoom?.GetComponentsInChildren<HotspotComponent>(true))).ConvertAll<IQuestScriptable>(item=>item.GetData());
		case eQuestObjectType.Region:    return (new List<RegionComponent>(m_selectedRoom?.GetComponentsInChildren<RegionComponent>(true))).ConvertAll<IQuestScriptable>(item=>item.GetData());
		}
		return null;
	}
