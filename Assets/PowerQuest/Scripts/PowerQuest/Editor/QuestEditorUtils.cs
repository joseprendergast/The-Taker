...
public class QuestEditorUtils
{
	...
	*/
	...
	public static bool OnFailOpenScript(string path, int line = 0)
	{ 
		Object scriptObj = null;
		if ( PowerQuestEditor.GetPowerQuestEditor().GetSmartCompileRequired() && Application.isPlaying == false )
		{				
			if ( EditorUtility.DisplayDialog("New file", "Before viewing the c# Unity needs to do an import/compile (takes a few seconds).", "Do it", "Never mind") )
			{
	...
	public static bool OpenScriptFunction(GameObject gameObject, string fileName, string fileTemplateText, string functionName, string parameters="", bool isCoroutine = true )
	{
		...
		if ( line >= 0 )
		{
			...
			{
				return OnFailOpenScript(path, line);
			}		
		}
		return false;
	}	
...
/*
	// If we get a "The imported type `blah` is defined multiple times" thing again, use this to figure out what assemblies the symbols are coming from 
	// so we can add them to the 'ignoreAssemblies' array. (Requires 'using System.Linq' commented out above)

	public static void LogReferences(string[] typeNames)
	{
		foreach (string typeName in typeNames)
		{
			var assembliesWithTypes = System.AppDomain.CurrentDomain.GetAssemblies()
				.Where(a => !a.IsDynamic) // Skip dynamic assemblies to avoid errors
				.SelectMany(a => a.GetTypes())
				.Where(t => t.FullName == typeName)
				.Select(t => t.Assembly.FullName)
				.ToList();

			foreach (var asm in assembliesWithTypes)
			{
				Debug.Log($"{typeName}: {asm}");
			}
		}
	}
*/
	...
	public static Assembly CompileFiles(string[] paths)
	{
		...
		string[] ignoreAssemblies = {"ARModule","ClothModule","AIModule","Terrain","Web","VRModule","Vehicles","Wind","XR","Google",".VR",".OSX","Linux","Plastic","Newtonsoft","mscorlib"}; // - don't think this makes a difference
		...
		CompilerResults result = provider.CompileAssemblyFromSource(param,sources);
		...
		if (result.Errors.Count > 0)
		{
			...
			foreach (CompilerError error in result.Errors) 
			{
				...
				// Filenames are temporary but have number matching index of paths. Eg: zxc782lk.123.cs  We need the 123				
				{ 
					Match match = Regex.Match(errorFileName, @"\.(\d+)\.cs"); // not bothering to compile this since its just for rare errors
					if ( match.Success && match.Groups.Count >= 2 )				
					{ 
						int errorFileId = int.Parse(match.Groups[1].Value);
						if ( errorFileId < paths.Length )
							errorFileName = paths[errorFileId];
					}
				}
				...
				mUnityLog.Invoke(null, new object[] { 
					string.Format("{0}({1},{2}): error {3}: {4}", errorFileName, error.Line, error.Column, error.ErrorNumber, error.ErrorText), 
					errorFileName,  error.Line, error.Column });
...
// New Object Window
class CreateQuestObjectWindow : PopupWindowContent 
{
	...
	bool m_createsChildOnly = true; // True if creating in a room
	string m_examples = null;
	bool m_addCollider = false;
	DelegateCreateFunction m_createFunction = null;	
	bool m_firstDraw = false;

	public static void Show(eQuestObjectType type, string typeDescription, string examples, DelegateCreateFunction createFunction, string path = null, bool createsChildOnly = false)
	{
		(new CreateQuestObjectWindow()).ShowQuestWindow(type, typeDescription, examples, createFunction, path, createsChildOnly);
	}
	...
	public void ShowQuestWindow(eQuestObjectType type, string typeDescription, string examples, DelegateCreateFunction createFunction, string path = null, bool createsChildOnly = false)
	{
		...
		if ( Application.isPlaying )
		{ 
			EditorUtility.DisplayDialog("Not while playing you can't", $"You can't add a new {typeDescription} while the game is running. Stop testing first","Oh yeah, oops");
		}
		else 
		{ 
			PopupWindow.Show(new Rect(),this);
		}
	}

	override public void OnGUI(Rect rect) 
	{
		editorWindow.minSize = new Vector2(400,80);
		editorWindow.maxSize = new Vector2(400,80);

		if ( string.IsNullOrEmpty(m_path) )
			m_path = QuestEditorUtils.FindCurrentPath();

		GUI.SetNextControlName("TxtNewObjName");
		m_prefabName = EditorGUILayout.TextField("Choose "+m_typeDescription+" Name", m_prefabName).Trim();
		string error;
		bool fail = !PowerQuestEditor.Get.SanitizeQuestObjectName(m_type, ref m_prefabName, out error);

		if ( m_createsChildOnly )
		...
		if ( fail && IsString.NotEmpty(m_prefabName) )
			EditorGUILayout.HelpBox(error,MessageType.Error);
		else if ( m_createsChildOnly == false && IsString.NotEmpty(m_prefabName) )
		{ 
			EditorGUILayout.LabelField("Create "+m_path+"/"+m_prefabName+"/"+m_typeDescription+m_prefabName+".prefab ?");
		}

		if ( m_type == eQuestObjectType.Prop )
		...
		if (m_firstDraw && Event.current.type == EventType.Repaint )
		{ 	
			rect.position = EditorGUIUtility.GUIToScreenPoint(Event.current.mousePosition.WithOffset(-390,-70));			
			editorWindow.position = rect;
			EditorGUI.FocusTextInControl("TxtNewObjName");
			m_firstDraw = false;
		}
		...
		if (create) 
		{
			if (m_type == eQuestObjectType.Prop )
			{ 
				PowerQuestEditor.CreateProp(m_prefabName, m_addCollider); //  Props are special. Yaay
			}
			else if ( m_createFunction == null )
			{
